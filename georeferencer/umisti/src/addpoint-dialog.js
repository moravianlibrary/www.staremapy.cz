...
georeferencer.umisti.AddPointDialog.prototype.enterDocument = function() {
  ...
  var northTooltipText = 'Vzdálenost v souřadnicovém systému severně (kladná) nebo <br> jižně (záporná), měřená od východozápadní referenční linie.';
  var eastTooltipText = 'Vzdálenost v souřadnicovém systému východně (kladná) <br> nebo západně (záporná) od severojižní referenční linie.';
  var projTooltipText = 'Souřadnicový systém umožňuje popsat polohu míst pomocí údajů <br> o zeměpisné šířce a délce. Definice jsou převzaty z databáze <a href="http://epsg.io/" target="_blank">epsg.io</a>, <br> ve které je každý systém označen jednoznačným identifikátorem.';

  var northTooltip = new goog.ui.Tooltip(northLabel);
  var eastTooltip = new goog.ui.Tooltip(eastLabel);
  var projTooltip = new goog.ui.Tooltip(projLabel);

  northTooltip.setHtml(northTooltipText);
  eastTooltip.setHtml(eastTooltipText);
  projTooltip.setHtml(projTooltipText);
};
...
georeferencer.umisti.AddPointDialog.coorInputHandler = function(e) {
  ...
  if (isMinus && startIsEmpty && !endContainsMinus) {
    ...
    if (!goog.string.contains(number, '.') && !goog.string.contains(number, ',')) {
